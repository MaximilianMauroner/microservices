...
describe("HTTP checker",()=>{
  ...
  it("rejects a redirect to a private address without retaining the redirect status",async()=>{const result=await checkTarget("https://example.com",{fetcher:async()=>new Response(null,{status:302,headers:{location:"http://127.0.0.1"}})});expect(result).toMatchObject({errorCode:"blocked_address",statusCode:null});});
  ...
  it("rejects a second redirect without reporting it as HTTP 302",async()=>{const result=await checkTarget("https://example.com",{fetcher:async()=>new Response(null,{status:302,headers:{location:"https://example.com/again"}})});expect(result).toMatchObject({errorCode:"too_many_redirects",statusCode:null});});
  it("uses one overall timeout across redirects without retaining the redirect status",async()=>{let calls=0;const fetcher:typeof fetch=async(_input,init)=>{calls+=1;if(calls===1)return new Response(null,{status:302,headers:{location:"https://example.com/slow"}});return new Promise((_resolve,reject)=>init?.signal?.addEventListener("abort",()=>reject(new DOMException("aborted","AbortError"))));};const result=await checkTarget("https://example.com",{fetcher,timeoutMs:5});expect(result).toMatchObject({errorCode:"timeout",statusCode:null});expect(calls).toBe(2);});
