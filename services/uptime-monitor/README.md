...
5. Deploy preview with `bunx wrangler deploy --env preview`, validate it, then deploy production with `bunx wrangler deploy --env production`. Verify the `* * * * *` trigger, assets route, D1 binding, Access policy, and custom domain in each environment.
...
To stop checks immediately, disable the environment's cron trigger in Cloudflare, or temporarily set `triggers.crons` to `[]` and run `bunx wrangler deploy --env <preview|production>`. Re-enable it with `["* * * * *"]` and deploy the same environment. Roll back Worker code with `bunx wrangler rollback --env <preview|production>`; inspect the target deployment before confirming. Use D1 Time Travel before reverting a destructive schema change; migrations are forward-only and should normally be corrected with a new migration. Deleting a monitor cascades its checks and incidents and should be treated as irreversible outside D1 recovery. Keep Access enabled during rollback.
