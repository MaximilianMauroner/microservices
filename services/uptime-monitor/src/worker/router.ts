...
function invalid(error:unknown):Response{const code=error instanceof Error?error.message:"invalid_request";const messages:Record<string,string>={invalid_name:"Name must be 1–80 characters",invalid_body:"Request body must be an object",invalid_enabled:"Enabled must be true or false",empty_patch:"Provide a field to update",invalid_content_type:"Content-Type must be application/json"};return problem(code,messages[code]??"Invalid request",400);}
...
export async function routeApi(request:Request,env:Env,ctx:ExecutionContext):Promise<Response>{
  ...
  try{
    ...
    if(request.method==="POST"&&url.pathname==="/api/monitors"){
      ...
      try{const monitor=await createMonitor(env.DB,input.name,input.url);ctx.waitUntil(runMonitorCheck(env,monitor.id));return json(monitor,201);}catch(error){if(error instanceof Error&&error.message==="monitor_limit_reached")return problem("monitor_limit_reached","All 40 monitor slots are full",409);if(error instanceof Error&&error.message.includes("UNIQUE"))return problem("duplicate_url","That URL is already monitored",409);throw error;}
    }
  ...
  }catch(error){if(error instanceof SyntaxError||error instanceof Error&&["invalid_name","invalid_body","invalid_enabled","empty_patch","invalid_content_type","network_error"].includes(error.message))return invalid(error);return problem("internal_error","The request could not be completed",500);}
}
