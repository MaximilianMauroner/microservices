...
export async function startServer(dependencies:{config?:Config;createRepository?:(config:Config)=>Promise<RepositoryHandle>;serve?:Serve;stylesheet?:BodyInit|Blob;log?:(message:string)=>void}={}) {
  ...
  try {
    ...
    const app=createApp({repository:handle.repository,agentAuth:agentAuth(config.agentApiToken),reviewerAuth:shooAuth({allowedEmail:config.allowedEmail,audience:`origin:${config.publicBaseUrl}`}),publicBaseUrl:config.publicBaseUrl,stylesheet});
