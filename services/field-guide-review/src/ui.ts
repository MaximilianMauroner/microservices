...
<html lang="en" class="bg-stone-950">
  <head>
    ...
    <script src="https://shoo.dev/shoo.js" nonce="${nonce}" defer></script>
  </head>
...
async function startSignIn(){if(!window.Shoo){showAuthMessage('Google sign-in is unavailable. Refresh and try again.',true);return}try{showAuthMessage('Opening secure sign-in…');await window.Shoo.startSignIn({redirectUri:location.origin+'/review/callback',audience:'origin:'+location.origin,requestPii:true})}catch(error){showAuthMessage(error.message||'Sign in failed.',true)}}
async function boot(){setAuthenticated(false);updateNavigation();if(!window.Shoo){showAuthMessage('Google sign-in could not load. Refresh and try again.',true);return}try{if(window.Shoo.parseCallback())await window.Shoo.handleCallback({redirectTo:'/review'});const identity=window.Shoo.getIdentity();if(!identity){showAuthMessage('Sign in to review lessons.');return}state.identity=identity;state.token=identity.token;elements.account.textContent=identity.email||'Reviewer';setAuthenticated(true);await loadReviews(false)}catch(error){showAuthMessage(error.message||'Authentication failed.',true)}}
