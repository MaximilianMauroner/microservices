...
describe("logical transfer",()=>{
  ...
  it("performs idempotent, authorized, atomic SQLite transfers and preserves the actual next sequence",async()=>{
    ...
    const decision=await sourceRepository.decide(first.candidateId,1,{action:"reject"},new Date(first.createdAt),"réviseur@example.com");
    ...
    const snapshot=sqliteSnapshot(source.client);expect(snapshot.sequence).toEqual({lastValue:"12",isCalled:true,nextValue:"13"});

    const target=temporary();const firstReport=transferSnapshotToSQLite(target.client,snapshot);expect(firstReport).toEqual(snapshotReport(sqliteSnapshot(target.client)));expect(transferSnapshotToSQLite(target.client,snapshot)).toEqual(firstReport);expect(snapshotsEqual(snapshot,sqliteSnapshot(target.client))).toBe(true);
...
const databaseUrl=process.env.TEST_DATABASE_URL;const confirmed=process.env.FIELD_GUIDE_TEST_DATABASE_CONFIRM==="field-guide-review-test";const gatedIt=databaseUrl&&confirmed?it:it.skip;
gatedIt("round-trips PostgreSQL through SQLite with microseconds, authorization, circular FKs, and sequence state",async()=>{
  ...
  try{
    ...
    await clear();
    const candidateId="55555555-5555-4555-8555-555555555555";const decisionId="66666666-6666-4666-8666-666666666666";const payload=candidate(candidateId,"微秒 会議");
    await database`INSERT INTO candidates VALUES(${candidateId},${"pg-unicode-鍵"},${database.json(payload)},${"hash"},${"2026-07-26T00:00:00.123456Z"})`;await database`INSERT INTO review_rounds(candidate_id,round,kind,due_at,verdict_id) VALUES(${candidateId},1,'initial',NULL,NULL)`;await database`INSERT INTO verdict_events(sequence,decision_id,candidate_id,round,action,reviewer,reviewed_at,next_review_at,round_kind,effect,amends_decision_id) VALUES(7,${decisionId},${candidateId},1,'reject',${"réviseur@example.com"},${"2026-07-26T01:02:03.654321Z"},NULL,'initial','deactivate',NULL)`;await database`UPDATE review_rounds SET verdict_id=${decisionId} WHERE candidate_id=${candidateId} AND round=1`;await database`INSERT INTO application_receipts VALUES(${"receipt"},${"receipt-hash"},${decisionId},${"2026-07-26T01:03:04.234567Z"},'applied')`;await database`INSERT INTO field_guide_schema_migrations VALUES(${"001_基準"},${"checksum"},${"2026-07-26T01:04:05.345678Z"},true)`;await database`SELECT setval(pg_get_serial_sequence('verdict_events','sequence'),40,true)`;
    ...
    await clear();await recoverSQLiteToPostgres(sqlite.client,url);const recovered=await postgresSnapshot(database);expect(snapshotsEqual(source,recovered)).toBe(true);expect(recovered.tables.verdict_events[0]?.reviewed_at).toBe("2026-07-26T01:02:03.654321Z");
