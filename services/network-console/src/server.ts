...
const server = serve({ fetch: app.fetch, hostname: bindHost, port }, (info) => {
  console.log(`network-console listening on ${info.address}:${info.port}`);
});
...
for (const signal of ["SIGINT", "SIGTERM"] as const) {
  process.once(signal, () => {
    ...
    shuttingDown = true;
    console.log(`received ${signal}; closing network-console`);
    server.close((error) => {
      if (error) {
        console.error("network-console shutdown failed", error);
        process.exitCode = 1;
      }
    });
  });
