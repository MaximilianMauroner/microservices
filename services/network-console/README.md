...
The checked-in `.service` files are public-safe examples. Use the install scripts so the real unit is generated with the current repo path, user, and pnpm executable on the VM.
...
### Unit rename migration

Before installing the renamed unit on a host that still runs
`tailscale-port-dashboard.service`, stop and disable that old unit so it cannot
compete for port 80:

```bash
sudo systemctl disable --now tailscale-port-dashboard.service
ops/install-systemd.sh
```

For a user-level installation, use the corresponding
`tailscale-port-dashboard.user.service` and `ops/install-user-service.sh`
commands. The network endpoint remains the same; only the local unit identity
changes. Roll back by stopping the new `network-console` unit and re-enabling
the previous unit from the prior checkout.
