...
export function MoneyCategoryExplorer({ spending, initialCategory }: { spending: MoneySpendingAnalytics; initialCategory?: MoneyCategory }) {
  ...
  return <div className="money-category-explorer space-y-3">
    ...
    <section className="grid items-start gap-3 lg:grid-cols-2">
      ...
      <Card><CardHeader className="border-b"><div className="flex items-start justify-between gap-3"><div><CardTitle>Recent transactions</CardTitle><CardDescription>Latest ledger rows in this category and selected period</CardDescription></div><Link to="/money" search={{ view: "transactions", category, fromMonth: selectedMonths[0], toMonth: selectedMonths.at(-1) }} className="money-inline-link">Open ledger</Link></div></CardHeader><CardContent className="p-0">{activity.length ? <><div className="divide-y md:hidden" role="list">{activity.map((row) => <article className="flex min-h-16 items-center justify-between gap-3 px-4 py-3" key={row.id} role="listitem"><div className="min-w-0"><h3 className="truncate text-sm font-medium">{row.description || row.sourceType}</h3><p className="mt-1 text-xs text-muted-foreground">{row.occurredAt.slice(0, 10)} · {row.categoryOrigin}</p></div><strong className="shrink-0 font-mono text-sm text-negative">{formatMinor(Math.abs(row.amountMinor))}</strong></article>)}</div><div className="hidden overflow-x-auto md:block"><table className="w-full min-w-[34rem] text-sm"><thead className="border-b text-xs text-muted-foreground"><tr><th className="px-4 py-3 text-left">Date</th><th className="px-4 py-3 text-left">Description</th><th className="px-4 py-3 text-left">Origin</th><th className="px-4 py-3 text-right">Amount</th></tr></thead><tbody className="divide-y">{activity.map((row) => <tr key={row.id}><td className="whitespace-nowrap px-4 py-3 text-muted-foreground">{row.occurredAt.slice(0, 10)}</td><td className="max-w-64 truncate px-4 py-3 font-medium">{row.description || row.sourceType}</td><td className="px-4 py-3"><Badge variant="outline">{row.categoryOrigin}</Badge></td><td className="px-4 py-3 text-right font-mono text-negative">{formatMinor(Math.abs(row.amountMinor))}</td></tr>)}</tbody></table></div></> : <EmptyState />}</CardContent></Card>
    </section>
  </div>;
}
