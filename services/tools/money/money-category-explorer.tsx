...
export function MoneyCategoryExplorer({ spending, initialCategory }: { spending: MoneySpendingAnalytics; initialCategory?: MoneyCategory }) {
  ...
  const activity = spending.categoryActivity.filter((row) => row.category === category);
  ...
  return <div className="space-y-3">
    ...
    <section className="grid items-start gap-3 lg:grid-cols-2">
      ...
      <Card><CardHeader className="border-b"><div className="flex items-start justify-between gap-3"><div><CardTitle>Recent transactions</CardTitle><CardDescription>Latest ledger rows in this category</CardDescription></div><Link to="/money" search={{ view: "transactions" }} className="money-inline-link">Open ledger</Link></div></CardHeader><CardContent className="overflow-x-auto p-0"><table className="w-full min-w-[34rem] text-sm"><thead className="border-b text-xs text-muted-foreground"><tr><th className="px-4 py-3 text-left">Date</th><th className="px-4 py-3 text-left">Description</th><th className="px-4 py-3 text-left">Origin</th><th className="px-4 py-3 text-right">Amount</th></tr></thead><tbody className="divide-y">{activity.map((row) => <tr key={row.id}><td className="whitespace-nowrap px-4 py-3 text-muted-foreground">{row.occurredAt.slice(0, 10)}</td><td className="max-w-64 truncate px-4 py-3 font-medium">{row.description || row.sourceType}</td><td className="px-4 py-3"><Badge variant="outline">{row.categoryOrigin}</Badge></td><td className="px-4 py-3 text-right font-mono text-rose-300">{formatMinor(Math.abs(row.amountMinor))}</td></tr>)}</tbody></table>{activity.length === 0 ? <EmptyState /> : null}</CardContent></Card>
    </section>
  </div>;
}
