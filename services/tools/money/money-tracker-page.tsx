...
export function MoneyTrackerPage(props: MoneyTrackerPageData & { view: MoneyTrackerView }) {
  ...
  return <><AppShell product="Money" showSignOut /><main id="main" className="app-page space-y-4">
    ...
    {props.view === "balances" ? <><MoneyBalanceEntry /><Accounts accounts={props.accounts} months={months} latest={latest} previous={previous} /><History accounts={props.accounts} months={months} /></> : null}
...
function Overview({ accounts, planning, months, latest, previous, monthlyChange, trends }: MoneyTrackerPageData & { months: GroupedMonth[]; latest?: GroupedMonth; previous?: GroupedMonth; monthlyChange?: number; trends: MoneyTrackerTrendStats }) {
...
function groupMonth(month: Month): GroupedMonth { let money = 0; let stocks = 0; for (const [account, value] of Object.entries(month.values)) { if (moneyTrackerAccountCategory(account) === "stocks") stocks += value; else money += value; } return { ...month, money, stocks, trend: month.total }; }
