...
import { moneyTrackerTrendStats, type MoneyTrackerAccountCategory, type MoneyTrackerTrendStats } from "./money-tracker-domain.js";
...
export function MoneyTrackerPage(props: MoneyTrackerPageData & { view: MoneyTrackerView }) {
  ...
  const allMonths = useMemo(() => props.months.map((month) => groupMonth(month, props.accountRoles)), [props.accountRoles, props.months]);
...
function Overview({ accounts, accountLabels, accountRoles, planning, months, latest, previous, monthlyChange, trends }: MoneyTrackerPageData & { months: GroupedMonth[]; latest?: GroupedMonth; previous?: GroupedMonth; monthlyChange?: number; trends: MoneyTrackerTrendStats }) {
...
function Accounts({ accounts, accountLabels, accountRoles, months, latest, previous }: { accounts: string[]; accountLabels: Record<string, string>; accountRoles: Record<string, "cash" | "investment">; months: GroupedMonth[]; latest?: GroupedMonth; previous?: GroupedMonth }) {
  const first = months.at(0);
  const cashAccounts = accounts.filter((account) => roleCategory(accountRoles, account) === "money");
  const stockAccounts = accounts.filter((account) => roleCategory(accountRoles, account) === "stocks");
  ...
  return <>
    ...
    <Card><CardHeader className="border-b"><CardTitle>Account detail</CardTitle><CardDescription>Latest movement, selected-period change, share, and observed range</CardDescription></CardHeader><CardContent className="overflow-x-auto p-0"><table className="w-full min-w-[64rem] text-sm"><thead className="border-b text-xs text-muted-foreground"><tr><th className="px-4 py-3 text-left">Account</th><th className="px-4 py-3 text-left">Category</th><th className="px-4 py-3 text-right">Balance</th><th className="px-4 py-3 text-right">Latest</th><th className="px-4 py-3 text-right">Latest %</th><th className="px-4 py-3 text-right">Selected period</th><th className="px-4 py-3 text-right">Period %</th><th className="px-4 py-3 text-right">Share</th><th className="px-4 py-3 text-right">Observed range</th></tr></thead><tbody className="divide-y">{rows.map((row) => <tr key={row.account}><td className="px-4 py-3 font-medium">{row.label}</td><td className="px-4 py-3"><Badge variant="outline">{roleCategory(accountRoles, row.account) === "money" ? "Cash" : "Stocks"}</Badge></td><td className="px-4 py-3 text-right font-mono">{preciseCurrency.format(row.value ?? 0)}</td><td className={`px-4 py-3 text-right font-mono ${changeClass(row.change)}`}>{formatSigned(row.change, true)}</td><td className={`px-4 py-3 text-right font-mono ${changeClass(row.change)}`}>{formatPercent(row.change, row.previous)}</td><td className={`px-4 py-3 text-right font-mono ${changeClass(row.periodChange)}`}>{formatSigned(row.periodChange, true)}</td><td className={`px-4 py-3 text-right font-mono ${changeClass(row.periodChange)}`}>{formatPercent(row.periodChange, row.firstValue)}</td><td className="px-4 py-3 text-right font-mono">{latest?.total ? `${((row.value ?? 0) / latest.total * 100).toFixed(1)}%` : "—"}</td><td className="px-4 py-3 text-right font-mono text-xs text-muted-foreground">{rangeForAccount(months, row.account)}</td></tr>)}</tbody></table></CardContent></Card>
  </>;
}
...
export function History({ accounts, accountLabels, months }: { accounts: string[]; accountLabels: Record<string, string>; months: GroupedMonth[] }) {
  ...
  return <>
    ...
    <section className="grid items-start gap-3 lg:grid-cols-2">
      ...
      <Card><CardHeader className="border-b"><CardTitle>Data contract</CardTitle><CardDescription>What these analytics can assert</CardDescription></CardHeader><CardContent className="space-y-3 pt-5 text-sm"><p><strong>{accounts.length}</strong> observed accounts across <strong>{months.length}</strong> snapshots.</p><p><strong>Cash and investments</strong> use the persisted account role, never the account label.</p><p className="text-muted-foreground">Balance changes are not investment returns. Deposits, withdrawals, and market performance cannot be separated from snapshot values.</p><Alert><AlertTitle>Money-owned history</AlertTitle><AlertDescription>Imported running balances and manual snapshots are stored in the private Money ledger.</AlertDescription></Alert></CardContent></Card>
    </section>
  </>;
}
