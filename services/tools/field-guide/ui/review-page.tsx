...
export function CorrectionDetails({ correction }: { correction: NonNullable<DecisionRecordItem["record"]["correction"]> }) {
  const rejections = Object.entries(correction.higherLevelRejections);
  return <section aria-labelledby="correction-prevention-title" className="rounded-lg border bg-muted/30 p-4">
    <p id="correction-prevention-title" className="text-xs font-medium text-muted-foreground">Correction prevention</p>
    <dl className="mt-3 grid gap-3 text-sm">
      <div><dt className="font-medium">Failed invariant</dt><dd className="mt-1 text-muted-foreground">{correction.failedInvariant}</dd></div>
      <div><dt className="font-medium">Selected layer</dt><dd className="mt-1 text-muted-foreground">{correction.selectedLayer}</dd></div>
      <div><dt className="font-medium">Mechanism</dt><dd className="mt-1 text-muted-foreground">{correction.mechanism}</dd></div>
