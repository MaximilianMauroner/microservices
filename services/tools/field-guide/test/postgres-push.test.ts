...
describe("PostgreSQL schema push", () => {
  ...
  it("fails closed before Drizzle or a database connection when test confirmation is absent", async () => {
    await expect(execFileAsync(process.execPath, [tsxCli, "src/push-postgres.ts", "test"], {
    ...
    await expect(execFileAsync(process.execPath, [tsxCli, "-e", "import('./drizzle.postgres.config.ts')"], {
      cwd: serviceDirectory,
      env: {
        ...process.env,
