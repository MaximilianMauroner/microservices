...
type FormRow = { id: string; public_token: string; title: string; introduction: string; questions: unknown; status: FeedbackFormStatus; created_at: Date; updated_at: Date; response_count: number; unread_count: number };
...
export interface FeedbackRepository {
  ...
  updateForm(id: string, input: { title: string; introduction: string; questions: readonly FeedbackQuestion[] }): Promise<FeedbackForm | undefined>;
  ...
  createSubmission(form: FeedbackForm, answers: Readonly<Record<string, string>>): Promise<string>;
...
export function feedbackRepository(sql: Sql): FeedbackRepository {
  const formSelect = sql`select f.id, f.public_token, f.title, f.introduction, f.questions, f.status, f.created_at, f.updated_at,
  ...
  return {
    ...
    async createForm(input) {
      const now = new Date();
      const id = randomUUID();
      await sql`insert into tools.feedback_forms (id, public_token, title, introduction, questions, status, created_at, updated_at)
        values (${id}, ${publicToken()}, ${input.title}, ${input.introduction ?? DEFAULT_FEEDBACK_INTRODUCTION}, ${sql.json([...FEEDBACK_TEMPLATE])}, 'draft', ${now}, ${now})`;
    ...
    async updateForm(id, input) {
      await sql`update tools.feedback_forms set title = ${input.title}, introduction = ${input.introduction}, questions = ${sql.json([...input.questions])}, updated_at = ${new Date()} where id = ${id}`;
    ...
    async createSubmission(form, answers) {
      const id = randomUUID();
      await sql`insert into tools.feedback_submissions (id, form_id, question_snapshot, answers, submitted_at, review_state, follow_up_state)
        values (${id}, ${form.id}, ${sql.json([...form.questions])}, ${sql.json(answers)}, ${new Date()}, 'unread', ${answers.follow_up && answers.follow_up !== "No" ? "wanted" : "none"})`;
      return id;
    },
...
function formFromRow(row: FormRow): FeedbackForm { return { id: row.id, publicToken: row.public_token, title: row.title, introduction: row.introduction, questions: row.questions as FeedbackQuestion[], status: row.status, createdAt: row.created_at.toISOString(), updatedAt: row.updated_at.toISOString(), responseCount: row.response_count, unreadCount: row.unread_count }; }
