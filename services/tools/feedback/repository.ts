...
import {
  DEFAULT_FEEDBACK_INTRODUCTION,
  DEFAULT_GERMAN_TRANSLATION,
  FEEDBACK_TEMPLATE,
...
type FormRow = { id: string; public_token: string; title: string; introduction: string; questions: unknown; translations: unknown; status: FeedbackFormStatus; created_at: Date; updated_at: Date; response_count: number; unread_count: number };
...
export interface FeedbackRepository {
  ...
  createForm(input: { title: string; introduction?: string }): Promise<FeedbackForm>;
  getForm(id: string): Promise<FeedbackForm | undefined>;
  getPublicForm(token: string): Promise<FeedbackForm | undefined>;
  updateForm(id: string, input: { title: string; introduction: string; questions: readonly FeedbackQuestion[]; translations: Readonly<Record<string, FeedbackTranslation>> }): Promise<FeedbackForm | undefined>;
...
export function feedbackRepository(sql: Sql): FeedbackRepository {
  const formSelect = sql`select f.id, f.public_token, f.title, f.introduction, f.questions, f.translations, f.status, f.created_at, f.updated_at,
  ...
  return {
    ...
    async createForm(input) {
      const now = new Date();
      const id = randomUUID();
      await sql`insert into tools.feedback_forms (id, public_token, title, introduction, questions, translations, status, created_at, updated_at)
        values (${id}, ${publicToken()}, ${input.title}, ${input.introduction ?? DEFAULT_FEEDBACK_INTRODUCTION}, ${sql.json([...FEEDBACK_TEMPLATE])}, ${sql.json({ de: DEFAULT_GERMAN_TRANSLATION })}, 'draft', ${now}, ${now})`;
    ...
    async updateForm(id, input) {
      await sql`update tools.feedback_forms set title = ${input.title}, introduction = ${input.introduction}, questions = ${sql.json([...input.questions])}, translations = ${sql.json(input.translations)}, updated_at = ${new Date()} where id = ${id}`;
...
function formFromRow(row: FormRow): FeedbackForm { const translations = row.translations as FeedbackForm["translations"]; return { id: row.id, publicToken: row.public_token, title: row.title, introduction: row.introduction, questions: row.questions as FeedbackQuestion[], translations: { ...translations, de: translations.de ?? DEFAULT_GERMAN_TRANSLATION }, status: row.status, createdAt: row.created_at.toISOString(), updatedAt: row.updated_at.toISOString(), responseCount: row.response_count, unreadCount: row.unread_count }; }
