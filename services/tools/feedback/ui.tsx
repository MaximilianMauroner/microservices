      ...
      <div className="mt-7 grid gap-5 lg:grid-cols-[minmax(0,1fr)_23rem]">
        <div className="grid gap-6 self-start">
          <form className={`${card} p-5 sm:p-6`} onSubmit={save}>
            ...
            <div className="mt-5 grid gap-3"><h3 className="text-sm font-semibold">Questions</h3>{questions.length ? questions.map((question, index) => <label className="grid gap-1.5 text-xs font-medium text-muted-foreground" key={question.id}>Question {index + 1}<textarea className={`${input} min-h-20 text-foreground`} maxLength={300} value={question.prompt} onChange={(event) => setQuestions((current) => current.map((item) => item.id === question.id ? { ...item, prompt: event.target.value } : item))} /></label>) : <div className="grid min-h-52 place-items-center rounded-xl border border-dashed bg-background/45 p-6 text-center"><div><div className="mx-auto grid size-12 place-items-center rounded-xl border border-primary/25 bg-primary/10 text-2xl text-primary">+</div><h3 className="mt-3 font-semibold text-foreground">No questions yet</h3><p className="mx-auto mt-1 max-w-sm text-sm text-muted-foreground">Use the schema prompt to create a complete question set, then paste the returned JSON.</p></div></div>}</div>
