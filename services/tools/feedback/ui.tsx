...
export function FeedbackFormPage({ form, submissions, publicOrigin }: { form: FeedbackForm; submissions: readonly FeedbackSubmission[]; publicOrigin: string }) {
  ...
  return <>
    ...
    <main id="main" className="mx-auto w-[min(1120px,calc(100%_-_2rem))] pb-20 pt-8">
      ...
      <div className="mt-6 flex gap-2" role="tablist" aria-label="Feedback form sections"><button type="button" role="tab" aria-selected={activePanel === "responses"} aria-controls="feedback-responses" className={`${button} ${activePanel === "responses" ? "bg-primary text-primary-foreground" : ""}`} onClick={() => setActivePanel("responses")}>Responses ({submissions.length})</button><button type="button" role="tab" aria-selected={activePanel === "edit"} aria-controls="feedback-form-editor" className={`${button} ${activePanel === "edit" ? "bg-primary text-primary-foreground" : ""}`} onClick={() => setActivePanel("edit")}>Edit form{unsaved ? " · Unsaved" : ""}</button></div>
      <section className={`mt-5 ${activePanel === "responses" ? "lg:hidden" : "hidden"}`} aria-label="Feedback overview">
        <div className="grid grid-cols-[1.2fr_1fr] gap-2"><button className={`${button} bg-primary text-primary-foreground hover:bg-primary/90`} type="button" onClick={() => setPreviewOpen(true)}>Preview form</button><button className={button} type="button" onClick={() => void copyFeedbackText(publicUrl, "Link copied")}>Copy link</button></div>
