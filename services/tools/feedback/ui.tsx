...
import { PublicFeedbackPage } from "./public-page.js";
...
const dangerButton = buttonVariants({ variant: "destructive-outline", size: "lg" });
const input = "w-full rounded-lg border border-input bg-background px-3 py-2.5 text-sm outline-none transition-colors focus:border-primary/60";
...
export function FeedbackFormPage({ form, submissions, publicOrigin }: { form: FeedbackForm; submissions: readonly FeedbackSubmission[]; publicOrigin: string }) {
  ...
  return <>
    ...
    <main id="main" className="mx-auto w-[min(1120px,calc(100%_-_2rem))] pb-20 pt-8">
      ...
      <div className="mt-6 flex gap-2" role="tablist" aria-label="Feedback form sections"><button type="button" role="tab" aria-selected={activePanel === "responses"} aria-controls="feedback-responses" className={activePanel === "responses" ? primaryButton : button} onClick={() => setActivePanel("responses")}>Responses ({submissions.length})</button><button type="button" role="tab" aria-selected={activePanel === "edit"} aria-controls="feedback-form-editor" className={activePanel === "edit" ? primaryButton : button} onClick={() => setActivePanel("edit")}>Edit form{unsaved ? " · Unsaved" : ""}</button></div>
      ...
      <div className="mt-7 grid gap-5 lg:grid-cols-[minmax(0,1fr)_23rem]">
        <div className="grid gap-6 self-start">
          <form id="feedback-form-editor" className={`${card} ${activePanel === "edit" ? "block" : "hidden"} p-5 sm:p-6`} onSubmit={save}>
            ...
            {error ? <p className="mt-4 text-sm text-destructive">{error}</p> : null}{unsaved ? <p className="mt-4 text-sm text-warning" role="status">Unsaved changes</p> : null}<button className={cn(primaryButton, "mt-5 w-full")} disabled={busy}>Save form</button>
          </form>
        ...
        <aside className="grid gap-4 self-start">
          ...
          <details className="rounded-2xl border border-emerald-500/25 bg-emerald-500/[0.05] p-5" open><summary className="cursor-pointer font-semibold">Schema prompt <span className="ml-2 rounded-full border border-emerald-500/25 px-2 py-0.5 text-[10px] font-medium text-positive">Version 2</span></summary><p className="mt-3 text-sm text-muted-foreground">The prompt collects the form topic, audience, goals, constraints, language, and tone before generating it.</p><div className="mt-3 rounded-xl border border-emerald-500/20 bg-background/70 p-3 font-mono text-xs leading-5 text-positive">Ask before writing JSON:<br />language · topic · audience · goals · constraints · tone</div><div className="mt-3 grid grid-cols-2 gap-2"><button className={button} type="button" onClick={() => copySchema("json")}>Copy JSON</button><button className={primaryButton} type="button" onClick={() => copySchema("prompt")}>Copy prompt</button></div><textarea className={`${input} mt-3 min-h-48 font-mono text-xs`} placeholder="Paste Feedback schema JSON here" value={schemaText} onChange={(event) => setSchemaText(event.target.value)} /><button className={cn(button, "mt-2 w-full")} type="button" disabled={!schemaText.trim()} onClick={applySchema}>Apply pasted JSON</button>{schemaNotice ? <p className="mt-2 text-xs text-muted-foreground" role="status">{schemaNotice}</p> : null}</details>
          <section className={`${card} p-5`}><h2 className="font-semibold">Public link</h2><p className="mt-1 break-all text-xs text-muted-foreground">{publicUrl}</p><div className="mt-3 grid grid-cols-2 gap-2"><button className={button} type="button" onClick={() => void copyFeedbackText(publicUrl, "Link copied")}>Copy link</button><button className={button} type="button" onClick={() => setPreviewOpen(true)}>Preview form</button></div><div className="mt-2 grid grid-cols-2 gap-2"><button className={button} disabled={busy} onClick={() => run(() => setFeedbackFormStatus({ data: { formId: form.id, status: form.status === "active" ? "closed" : "active" } }))}>{form.status === "active" ? "Close" : "Activate"}</button><button className={button} disabled={busy} onClick={() => window.confirm("Rotate this link? The current link will stop working.") && run(() => rotateFeedbackToken({ data: { formId: form.id } }))}>Rotate link</button></div><a className={cn(button, "mt-2 w-full")} href={`/api/feedback/forms/${form.id}/export`}>Export CSV</a></section>
          <section className="rounded-2xl border border-destructive/35 p-5"><h2 className="font-semibold text-destructive">Delete form</h2><p className="mt-1 text-sm text-muted-foreground">This permanently deletes every response.</p><button className={cn(dangerButton, "mt-3 w-full")} disabled={busy} onClick={remove}>Delete permanently</button></section>
          </div>
        </aside>
      </div>
      <Dialog open={previewOpen} onOpenChange={setPreviewOpen}><DialogContent className="max-h-[90vh] w-[min(760px,calc(100vw_-_2rem))] overflow-y-auto"><DialogHeader><DialogTitle>Form preview</DialogTitle><DialogDescription>Shows your current editor changes. Preview answers are never submitted.</DialogDescription></DialogHeader><PublicFeedbackPage form={localizeFeedbackForm({ ...form, language, title, introduction, questions })} submitted={false} preview /></DialogContent></Dialog>
    </main>
  </>;
}
