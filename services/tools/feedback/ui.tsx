...
import type { FeedbackForm, FeedbackFollowUpState, FeedbackQuestion, FeedbackReviewState, FeedbackSubmission } from "./domain.js";
import {
  createFeedbackForm,
  deleteFeedbackForm,
...
export function FeedbackFormPage({ form, submissions, publicOrigin }: { form: FeedbackForm; submissions: readonly FeedbackSubmission[]; publicOrigin: string }) {
  ...
  async function save(event: FormEvent) { event.preventDefault(); await run(() => updateFeedbackForm({ data: { formId: form.id, title, introduction, questions } })); }
    ...
    <main id="main" className="mx-auto w-[min(1080px,calc(100%_-_2rem))] pb-20 pt-8">
      ...
      <div className="mt-5 grid gap-8 lg:grid-cols-[1fr_23rem]">
        ...
        <aside className="grid gap-4 self-start">
          ...
          <form className="rounded-xl border bg-card p-5" onSubmit={save}><h2 className="font-semibold">Form text</h2><label className="mt-4 grid gap-1 text-sm font-medium">Title<input className={input} maxLength={120} value={title} onChange={(event) => setTitle(event.target.value)} /></label><label className="mt-3 grid gap-1 text-sm font-medium">Introduction<textarea className={`${input} min-h-36`} maxLength={2000} value={introduction} onChange={(event) => setIntroduction(event.target.value)} /></label><div className="mt-4 grid gap-3"><h3 className="text-sm font-semibold">Questions</h3>{questions.map((question, index) => <label className="grid gap-1 text-xs font-medium text-muted-foreground" key={question.id}>Question {index + 1}<textarea className={`${input} min-h-20 text-foreground`} maxLength={300} value={question.prompt} onChange={(event) => setQuestions((current) => current.map((item) => item.id === question.id ? { ...item, prompt: event.target.value } : item))} /></label>)}</div>{error ? <p className="mt-3 text-sm text-destructive">{error}</p> : null}<button className={`${button} mt-4 w-full`} disabled={busy}>Save form</button></form>
