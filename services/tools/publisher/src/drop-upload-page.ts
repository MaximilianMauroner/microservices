...
try{const data=new FormData();data.append('file',file);const path=location.pathname.replace(/\\/$/,'').replace(/^\\/drop\\//,'/api/drop/')+'/uploads';const response=await fetch(path,{method:'POST',body:data,headers:{Accept:'application/json'}});const payload=await response.json().catch(()=>({}));if(!response.ok)throw new Error(payload.message||'Upload failed.');item.className='ok';item.textContent='Uploaded '+file.name;ok++;}catch(error){item.className='error';item.textContent=file.name+': '+(error instanceof Error?error.message:'Upload failed.');}}
