...
async function completeUpload(sessionPath){for(let attempt=0;attempt<4;attempt++){const response=await fetch(sessionPath+'/complete',{method:'POST',headers:{Accept:'application/json'}});if(response.status!==503)return readResponse(response);if(attempt<3){await new Promise(resolve=>setTimeout(resolve,retryDelay(response)));continue}const payload=await response.json().catch(()=>({})),error=new Error(payload.message||'Upload is ready, but the server is busy. Please try again.');error.preserveSession=true;throw error}}
