...
import { Empty as EmptyRoot, EmptyContent, EmptyDescription, EmptyHeader, EmptyMedia, EmptyTitle } from "../../components/ui/empty.js";
...
function ReviewNav({ search }: { search: ReviewSearch }) {
  return <Tabs value={search.view}><TabsList>{(["decisions", "queue", "history"] as const).map((view) => <TabsTrigger key={view} value={view} render={<Link to="/review" search={{ ...search, view }} preload="intent" />}>{view === "decisions" ? "Decisions" : view === "queue" ? "Candidates" : "History"}</TabsTrigger>)}</TabsList></Tabs>;
}
  ...
  return <>
    <div className="mb-4 flex flex-col gap-3 lg:flex-row lg:items-center lg:justify-between">
      <div className="flex flex-wrap items-center gap-2">
        ...
        <Tabs value={search.reviewState}><TabsList>{(["unreviewed", "reviewed", "all"] as const).map((reviewState) => <TabsTrigger key={reviewState} value={reviewState} render={<Link to="/review" search={{ ...search, reviewState }} preload="intent" />}>{reviewState === "all" ? "All" : reviewState === "reviewed" ? "Reviewed" : "Unreviewed"}</TabsTrigger>)}</TabsList></Tabs>
      </div>
...
function ReviewScopeTabs({ search }: { search: ReviewSearch }) {
  return <Tabs value={search.scope}><TabsList><TabsTrigger value="project" render={<Link to="/review" search={{ ...search, scope: "project" }} preload="intent" />}>Project</TabsTrigger><TabsTrigger value="global" render={<Link to="/review" search={{ ...search, scope: "global" }} preload="intent" />}>Global</TabsTrigger></TabsList></Tabs>;
}
