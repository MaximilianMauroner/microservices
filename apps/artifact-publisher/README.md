...
Railway deploys this service with `apps/artifact-publisher` as the service root.
`railway.json` starts the compiled server and uses `/health` as a liveness
check. The existing `uploads.mauroner.eu` domain and storage contract are
unchanged. Shutdown stops new requests, gives active requests 10 seconds to
drain, and then closes S3 connections.
