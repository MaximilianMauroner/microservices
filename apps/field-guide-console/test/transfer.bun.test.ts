...
describe("logical transfer",()=>{
  ...
  it("passes strict startup overwrite authorization and exposes only the safe report",async()=>{const handle=temporary();let received:{url:string;allow:boolean}|undefined;const report=snapshotReport(sqliteSnapshot(handle.client));const repositoryHandle=await createRepository({backend:"sqlite",sqlitePath:"/unused",databaseUrl:"postgres://user:secret@db/private",importOnStart:true,importAllowOverwrite:true,port:3000,agentApiToken:"agent-secret",publicBaseUrl:"https://review.example"},{openSQLite:()=>handle,importPostgresToSQLite:async(_database,url,allow)=>{received={url,allow:allow??false};return report;}});expect(received).toEqual({url:"postgres://user:secret@db/private",allow:true});expect(repositoryHandle.startupReport).toEqual(report);expect(JSON.stringify(report)).not.toContain("secret");expect(JSON.stringify(report)).not.toContain("private");await repositoryHandle.close();});
});
