...
import { sqliteSnapshot, summarize } from "../src/db/logical-snapshot.js";
...
describe("SQLite decision records",()=>{
  ...
  it("canonicalizes direct mixed-case UUIDs and preserves them through restore",async()=>{
    ...
    const snapshot=sqliteSnapshot(source.handle.client);expect(snapshot.tables.decision_records[0]).toMatchObject({decision_record_id:lowercaseId,payload:{decisionRecordId:lowercaseId}});const uppercaseCandidateId=candidateId.toUpperCase(),invalid=summarize({...snapshot.tables,candidates:snapshot.tables.candidates.map((row)=>row.candidate_id===candidateId?{...row,candidate_id:uppercaseCandidateId,payload:{...(row.payload as object),candidateId:uppercaseCandidateId}}:row),review_rounds:snapshot.tables.review_rounds.map((row)=>row.candidate_id===candidateId?{...row,candidate_id:uppercaseCandidateId}:row),decision_promotions:snapshot.tables.decision_promotions.map((row)=>({...row,candidate_id:uppercaseCandidateId})),decision_promotion_records:snapshot.tables.decision_promotion_records.map((row)=>({...row,candidate_id:uppercaseCandidateId}))},snapshot.sequences),rejected=setup();expect(()=>transferSnapshotToSQLite(rejected.handle.client,invalid)).toThrow("canonical UUID");expect(sqliteSnapshot(rejected.handle.client).counts.candidates).toBe(0);const target=setup();transferSnapshotToSQLite(target.handle.client,snapshot);expect((await target.repository.decisionRecord(uppercaseId,now,90)).record.decisionRecordId).toBe(lowercaseId);await source.repository.close();await rejected.repository.close();await target.repository.close();
  });
});
