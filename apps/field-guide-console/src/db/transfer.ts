...
async function writePostgres(tx:TransactionSql,snapshot:LogicalSnapshot){
  ...
  await setSequence(tx,"decision_records",snapshot.tables.decision_records);
  await setSequence(tx,"decision_feedback_events",snapshot.tables.decision_feedback_events);
}
async function setSequence(tx:TransactionSql,table:string,rows:readonly Record<string,unknown>[]){const next=rows.reduce((max,row)=>{const value=BigInt(String(row.sequence));return value>max?value:max;},0n)+1n;await tx.unsafe(`SELECT setval(pg_get_serial_sequence('${table}','sequence'),$1,false)`,[next.toString()]);}
